...
interface LegalPagesListCriteria {
...
const DEFAULT_CRITERIA: LegalPagesListCriteria = {
  organizationIds: [],
  search: null,
  isActive: null,
  page: 1,
...
@Injectable()
export class LegalPagesListFacade {
  #criteria: () => LegalPagesListCriteria = () => DEFAULT_CRITERIA;

  connect(criteria: () => LegalPagesListCriteria): void {
    this.#criteria = criteria;
  }

  readonly #listResource = httpResource<ApiPagedResponse<LegalPage>>(() => {
    const c = this.#criteria();
    return {
      url: `${environment.apiUrl}/v1/legal-pages`,
